...
class AnnotationCampaignViewSetTestCase(APITestCase):
    ...
    def test_retrieve(self):
        ...
        self.assertEqual(list(response.data['campaign'].keys()), [
            'id',
            'name',
            'desc',
            'instructions_url',
        ...
        self.assertEqual(dict(response.data['tasks'][0]), {'annotator_id': 1, 'count': 6, 'status': 0})
    ...
    def test_create(self):
        ...
        expected_reponse = { key:self.creation_data[key] for key in [
            'name', 'desc', 'instructions_url', 'start', 'end', 'annotation_set_id', 'datasets'
        ]}
