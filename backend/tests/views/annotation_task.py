...
class AnnotationTaskViewSetUnauthenticatedTestCase(APITestCase):
    ...
    def test_campaign_user_list_unauthenticated(self):
        ...
        url = reverse('annotation-task-campaign-user-list', kwargs={ 'campaign_id': 1, 'user_id': 1 })
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class AnnotationTaskViewSetTestCase(APITestCase):
    ...
    def setUp(self):
        self.client.login(username='user2', password='osmose29') # campaign annotator, user1 is owner
    ...
    def test_campaign_user_list_for_staff_plus_unknown_campaign(self):
        ...
        self.client.login(username='staff', password='osmose29')
        url = reverse('annotation-task-campaign-user-list', kwargs={ 'campaign_id': 42, 'user_id': 1 })
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_campaign_user_list_for_owner_plus_unknown_user(self):
        ...
        self.client.login(username='user1', password='osmose29')
        url = reverse('annotation-task-campaign-user-list', kwargs={ 'campaign_id': 1, 'user_id': 42 })
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_retrieve(self):
        ...
        self.assertEqual(response.data['annotationTags'], ['Mysticetes', 'Odoncetes', 'Boat', 'Rain', 'Other'])
        self.assertEqual(dict(response.data['boundaries']), {
