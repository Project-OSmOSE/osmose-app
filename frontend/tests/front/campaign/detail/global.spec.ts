...
test('annotator can view campaign global information ', async ({ annotatorPage }) => {
...
test('annotator can access campaign annotation file list', async ({ annotatorPage }) => {
  await accessCampaignDetail(annotatorPage);
  await canAccessCampaignFileList(annotatorPage, annotatorPage)
})

test('admin (not annotator) cannot access campaign annotation file list', async ({ adminPage }) => {
