...
class AudioAnnotator extends Component<AudioAnnotatorProps, AudioAnnotatorState> {
  ...
  componentDidMount() {
    const taskId: number = parseInt(this.props.match.params.annotation_task_id);
  ...
  componentDidUpdate(prevProps: AudioAnnotatorProps) {
    const prevTaskId: number = parseInt(prevProps.match.params.annotation_task_id);
    const taskId: number = parseInt(this.props.match.params.annotation_task_id);
      ...
      if (this.state.task) {
        ...
        if (event.key === value || event.key === this.alphanumeric_keys[1][index]) {
          ...
          if (this.state.annotations.length === 0) {
            ...
            let newcheckbox_isChecked: {[tag: string]: boolean} = this.state.checkbox_isChecked;
  ...
  checkAndSubmitAnnotations = () => {
    ...
    if (emptyAnnotations.length > 0) {
      this.activateAnnotation(emptyAnnotations[0]);
      this.setState({
  ...
  render() {
    if (this.state.isLoading) {
      ...
      if (this.audioPlayer && this.audioPlayer.audioElement && this.audioPlayer.audioElement.preservesPitch !== undefined) {
        playbackRateSelect = (
          <select
