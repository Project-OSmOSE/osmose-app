...
class AudioPlayer extends Component<AudioPlayerProps> {
  ...
  componentDidMount() {
    ...
    // Do not preserve pitch when changing playback rate (experimental, so prevent type errors)
  ...
  updateVolume(volume: number) {
    if (this.audioElement && typeof volume === 'number' && volume !== this.audioElement?.volume) {
      this.audioElement.volume = volume;
    }
  }

  updatePlaybackRate(rate: number) {
    if (this.audioElement && typeof rate === 'number' && rate !== this.audioElement.playbackRate) {
