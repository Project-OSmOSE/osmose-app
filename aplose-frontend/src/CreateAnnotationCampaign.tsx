...
type ShowAnnotationSetProps = {
  annotation_sets: {
    [key: number]: annotation_set_type
  },
...
class CreateAnnotationCampaign extends Component<CACProps, CACState> {
  ...
  getDatasets: any;
  getAnnotationSets: any;
  getUsers: any;
  postAnnotationCampaign: any;

  componentDidMount() {
    // TODO the following error handling is very messy
    // This should be fixed in a future rework of API calls
    this.getDatasets = request.get(GET_DATASETS_API_URL, this.props.app_token);
    this.getAnnotationSets = request.get(GET_ANNOTATION_SETS_API_URL, this.props.app_token);
    this.getUsers = request.get(GET_USERS_API_URL, this.props.app_token);
    return Promise.all([
      this.getDatasets.then((req: any) => {
      ...
      this.getAnnotationSets.then((req: any) => {
        this.setState({
  ...
  handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    this.postAnnotationCampaign = request.post(POST_ANNOTATION_CAMPAIGN_API_URL, this.props.app_token, res);
    return this.postAnnotationCampaign
    .then(() => {
