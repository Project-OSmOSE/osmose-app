...
class AnnotationTaskList extends Component<AnnotationTaskListProps, AnnotationTaskListState> {
  ...
  componentDidMount() {
    ...
    this.getCampaign = request.get(CAMPAIGN_API_URL + campaignID, this.props.app_token);
    this.getTasks = request.get(TASKS_API_URL.replace('ID', campaignID), this.props.app_token);
