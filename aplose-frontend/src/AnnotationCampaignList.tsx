...
class AnnotationCampaignList extends Component<ACLProps, ACLState> {
  ...
  render() {
    ...
    const annotation_campaigns = this.state.annotation_campaigns.map(
      (annotation_campaign) => {
        ...
        return (
          <tr key={annotation_campaign.id}>
            <td>
              <Link to={"/annotation_campaign/" + annotation_campaign.id}>
                {annotation_campaign.name}
              </Link>
            </td>
            <td>Set n°{annotation_campaign.annotation_set_id}</td>
            <td>{annotation_campaign.files_count}</td>
            <td>
              {annotation_campaign.start
                ? new Date(annotation_campaign.start).toDateString()
                : "N/A"}
            </td>
            <td>
              {annotation_campaign.end
                ? new Date(annotation_campaign.end).toDateString()
                : "N/A"}
            </td>
            <td>
              {annotation_campaign.user_complete_tasks_count} /{" "}
              {annotation_campaign.user_tasks_count}
            </td>
            <td>{instructions}</td>
            <td>
              <Link to={"/annotation_tasks/" + annotation_campaign.id}>
                My tasks
              </Link>
            </td>
          </tr>
      ...
      <div className="col-sm-9 border rounded">
        ...
        <p className="text-center">
          <a
            ...
            target="_blank"
          >
            <span className="fa fa-question-circle"></span>&nbsp;Annotator User
            Guide
          </a>
        </p>
        <table className="table table-bordered">
          ...
          <tbody>{annotation_campaigns}</tbody>
        </table>
        <p className="text-center">
          <Link to="/create-annotation-campaign" className="btn btn-primary">
